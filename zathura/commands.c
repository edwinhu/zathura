...
bool cmd_highlights_delete_embedded(girara_session_t* session, girara_list_t* GIRARA_UNUSED(argument_list)) {
  g_return_val_if_fail(session != NULL, false);
  g_return_val_if_fail(session->global.data != NULL, false);
  zathura_t* zathura = session->global.data;
  ...
  for (unsigned int page_id = 0; page_id < number_of_pages; page_id++) {
    ...
    /* Found a page with selected highlight */
    girara_debug("Found selected highlight: %s on page %u", selected_id, page_id);

    /* Check if this is a database highlight (not embedded) */
    if (zathura->database != NULL && file_path != NULL) {
      ...
      if (db_highlights != NULL) {
        ...
        GIRARA_LIST_FOREACH_BODY(db_highlights, zathura_highlight_t*, hl,
          if (hl->id != NULL && g_strcmp0(hl->id, selected_id) == 0) {
            found_in_db = true;
            break;
          }
        );
        ...
        if (found_in_db) {
          girara_notify(session, GIRARA_WARNING,
              _("This is a database highlight. Use 'x' key to delete."));
          return false;
        }
      }
    }

    /* Get the highlight geometry from page widget */
    girara_list_t* highlights = zathura_page_widget_get_highlights(ZATHURA_PAGE(page_widget));
    if (highlights == NULL) {
      girara_notify(session, GIRARA_ERROR, _("No highlights on page."));
      return false;
    }
    ...
    if (target_highlight == NULL || target_highlight->rects == NULL) {
      girara_notify(session, GIRARA_ERROR, _("Could not find highlight geometry."));
      return false;
    }

    /* Delete the annotation from PDF */
    zathura_error_t error = zathura_page_delete_annotation(page, target_highlight->rects);
    if (error != ZATHURA_ERROR_OK) {
      if (error == ZATHURA_ERROR_NOT_IMPLEMENTED) {
        girara_notify(session, GIRARA_ERROR,
            _("Annotation deletion not supported by this plugin."));
      } else {
        girara_notify(session, GIRARA_ERROR, _("Failed to delete annotation."));
      }
      return false;
    }

    /* Save the document to persist the deletion */
    error = zathura_document_save_as(zathura->document, file_path);
    if (error != ZATHURA_ERROR_OK) {
      girara_notify(session, GIRARA_ERROR, _("Deleted annotation but failed to save document."));
      return false;
    }
    ...
    girara_notify(session, GIRARA_INFO, _("Embedded annotation deleted."));
    return true;
  }
