...
static void zathura_database_default_init(ZathuraDatabaseInterface* GIRARA_UNUSED(iface)) {}
...
girara_list_t* zathura_db_load_quickmarks(ZathuraDatabase* db, const char* file) {
  ...
  return ZATHURA_DATABASE_GET_INTERFACE(db)->load_quickmarks(db, file);
}
